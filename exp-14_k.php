...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Inheritance and Method Overloading/Overriding in Python ---
        $question = [
            "topic" => "Inheritance and Method Overloading/Overriding in Python",
            "question" => "This assignment checks inheritance, method overriding, method 'overloading' emulation (using default args or *args), polymorphism and using super().\n\nYour program should read from standard input where the first token (single word) selects the operation: one of 'override', 'overload', 'polymorph', 'supercall'. For each operation the input format and expected behavior are below. Be exact with spacing and trailing newlines.\n\n1) 'override'\n   - No further input lines.\n   - Requirement: define a base class Animal with method speak(self) returning a string like 'Generic'. Define a subclass Dog that overrides speak(self) to return 'Woof!'. In main create a Dog instance and print the result of dog.speak() followed by a newline.\n   - Expected output: 'Woof!\\n'\n\n2) 'overload'\n   - Next line: either two or three integers separated by spaces (e.g. '2 3' or '1 2 3').\n   - Requirement: implement a Calculator class with an add method that accepts either two or three numbers (emulate overloading using default args or *args) and returns their sum. Read the integers from input, call add with the provided numbers, and print the integer sum followed by a newline.\n   - Example: input 'overload\\n2 3\\n' -> output '5\\n'\n\n3) 'polymorph'\n   - Next line: integer n (number of animals)\n   - Next n lines: each line is either 'Dog' or 'Cat'\n   - Requirement: implement classes Dog and Cat, both with a speak(self) method returning 'Woof!' and 'Meow!' respectively. In main, for each input line instantiate the appropriate class, call speak(), and print each result on its own line (preserve input order). After last line print a final newline.\n   - Example input:\n       polymorph\n       2\n       Dog\n       Cat\n     output:\n       Woof!\n       Meow!\n\n4) 'supercall'\n   - Next line: a single token name (no spaces)\n   - Requirement: implement a base class Employee with __init__(self, name) storing name and get_role(self) returning 'Employee'. Implement Manager(Employee) which calls super().__init__(name) in its constructor and overrides get_role(self) to return 'Manager'. Instantiate Manager with the given name and print '<name> <role>' followed by a newline. (Use super() in Manager.__init__ to show correct usage.)\n   - Example: input 'supercall\\nJohn\\n' -> output 'John Manager\\n'\n\nUse exact single spaces and trailing newlines. Implementations using Python OOP (classes, methods, inheritance, super, flexible argument techniques) will be accepted by the auto-grader.",
            "explanation" => "Covers method overriding, emulating method overloading in Python (via default params/*args), polymorphism by common method names across classes, and proper use of super() to chain constructors.",
            "test_cases" => [
                [
                    "input"  => "override\n",
                    "output" => "Woof!\n",
                    "desc"   => "Dog overrides Animal.speak -> Woof!",
                    "marks"  => 2.5
                ],
                [
                    "input"  => "overload\n2 3\n",
                    "output" => "5\n",
                    "desc"   => "Calculator.add handles two args (2+3=5).",
                    "marks"  => 2.5
                ],
                [
                    "input"  => "polymorph\n2\nDog\nCat\n",
                    "output" => "Woof!\nMeow!\n",
                    "desc"   => "Polymorphism: Dog and Cat speak produce respective outputs preserving order.",
                    "marks"  => 2.5
                ],
                [
                    "input"  => "supercall\nJohn\n",
                    "output" => "John Manager\n",
                    "desc"   => "Manager uses super() and overrides get_role -> 'John Manager'.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (strpos($desc, "overrides") !== false || strpos($desc, "override") !== false) {
                if (strpos($answer, 'class') !== false && strpos($answer, 'speak') !== false) $passed = true;
            } elseif (strpos($desc, "Calculator") !== false || strpos($desc, "overload") !== false) {
                if (strpos($answer, 'def add') !== false || strpos($answer, 'args') !== false || strpos($answer, 'add(') !== false) $passed = true;
            } elseif (strpos($desc, "Polymorphism") !== false || strpos($desc, "polymorph") !== false) {
                if (strpos($answer, 'dog') !== false && strpos($answer, 'cat') !== false) $passed = true;
            } elseif (strpos($desc, "Manager") !== false || strpos($desc, "super()") !== false) {
                if (strpos($answer, 'super') !== false || strpos($answer, 'get_role') !== false) $passed = true;
            } else {
