...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Recursive Functions: Prime Factorization, Subsets, Fibonacci ---
        $question = [
            "topic" => "Recursive Functions: Prime Factors, Subsets Generation, Fibonacci",
            "question" => "First token (single word) indicates the operation: one of 'prime', 'subsets', 'fibonacci'.\n\nOperation formats and required outputs:\n\n1) 'prime'\n   - Next line: single integer n.\n   - Requirement: using recursion (students are expected to write a recursive factorization), print the prime factors of n in non-decreasing order separated by single spaces, followed by a newline. If n < 2 print exactly 'INVALID' followed by a newline.\n   - Example: input 60 -> output: 2 2 3 5\n\n2) 'subsets'\n   - Next line: zero or more space-separated integers (the list). If the line is empty, treat it as an empty list.\n   - Requirement: generate all subsets (the power set) using recursion and print them one per line. Subsets must be ordered by increasing size, and within the same size by lexicographic order of elements (numerical ascending). Represent each subset as its elements separated by single spaces. For the empty subset print an empty line. Finally print a newline after the last subset line.\n   - Example for '1 2 3' the lines are:\n     (empty line)\n     1\n     2\n     3\n     1 2\n     1 3\n     2 3\n     1 2 3\n\n3) 'fibonacci'\n   - Next line: single integer n.\n   - Requirement: using recursion, print the first n Fibonacci numbers starting from 0 and 1, separated by single spaces on one line, then a newline. If n < 1, print 'INVALID' followed by a newline.\n   - Example: n=7 -> 0 1 1 2 3 5 8\n\nBe exact with spacing and newlines. The auto-grader expects integer arithmetic and deterministic ordering as described.",
            "explanation" => "Tests recursive problem solving: recursive prime factor decomposition, recursive generation of the power set (ordered by size then lexicographically), and recursive Fibonacci sequence generation.",
            "test_cases" => [
                // 1: prime factorization typical
                [
                    "input"  => "prime\n60\n",
                    "output" => "2 2 3 5\n",
                    "desc"   => "Prime factors of 60 (2 2 3 5).",
                    "marks"  => 2.5
                ],
                // 2: subsets for 1 2 3
                [
                    "input"  => "subsets\n1 2 3\n",
                    "output" => "\n1\n2\n3\n1 2\n1 3\n2 3\n1 2 3\n",
                    "desc"   => "Power set of [1,2,3] ordered by size then lexicographically; empty subset is first (empty line).",
                    "marks"  => 2.5
                ],
                // 3: fibonacci typical
                [
                    ...
                    "desc"   => "First 7 Fibonacci numbers starting from 0,1.",
                    "marks"  => 2.5
                ],
                // 4: prime invalid (n < 2)
                [
                    "input"  => "prime\n1\n",
                    "output" => "INVALID\n",
                    "desc"   => "n=1 is invalid for prime factorization -> INVALID.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (strpos($desc, "prime") !== false) {
                if (strpos($answer, 'prime') !== false || strpos($answer, 'factor') !== false) $passed = true;
            } elseif (strpos($desc, "subsets") !== false) {
                if (strpos($answer, 'subset') !== false || strpos($answer, 'power') !== false || strpos($answer, 'recurs') !== false) $passed = true;
            } elseif (strpos($desc, "fibonacci") !== false) {
                if (strpos($answer, 'fib') !== false || strpos($answer, 'fibonacci') !== false) $passed = true;
            } else {
