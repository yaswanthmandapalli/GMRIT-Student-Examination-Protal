...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Regular Expressions — Email, Phone, Password ---
        $question = [
            "topic" => "Regular Expressions: Email Validation, Phone Extraction, Password Strength",
            "question" => "First line: an operation token — one of: 'email', 'phone', 'password'.\n\nOperation formats & expected behavior:\n\n1) 'email'\n  - Next line: a single email string.\n  - Requirement: validate the email using a robust regular expression (accept common valid emails, allow subdomains, plus-tags, and TLDs). If the email is valid, print exactly 'VALID' followed by a newline; otherwise print 'INVALID' followed by a newline.\n\n2) 'phone'\n  - Next line: an arbitrary text string that may contain zero or more phone numbers in common formats (examples: +1-202-555-0173, (202) 555-0125, 202.555.0186, 2025550199).\n  - Requirement: extract all phone-like tokens and normalize each by removing all non-digit characters except preserve a leading '+' if present. Print each extracted normalized phone number on its own line in the order found. If none found, print an empty line (just a newline).\n\n3) 'password'\n  - Next line: a single password string.\n  - Requirement: check password strength. Print 'STRONG' followed by newline if the password is at least 8 characters long and contains at least one uppercase letter, one lowercase letter, one digit, and one special character (non-alphanumeric). Otherwise print 'WEAK' followed by newline.\n\nBe exact with spacing and newlines.",
            "explanation" => "Tests regular expression usage for practical validation and extraction tasks: email format validation, phone number extraction/normalization, and password strength rules.",
            "test_cases" => [
                // 1: valid email (plus-tag & subdomain)
                [
                    "input"  => "email\nuser.name+tag@sub.example.co.uk\n",
                    "output" => "VALID\n",
                    "desc"   => "Email with plus-tag and subdomain should be VALID.",
                    "marks"  => 2.5
                ],
                // 2: invalid email (double @)
                [
                    "input"  => "email\nuser@@example.com\n",
                    "output" => "INVALID\n",
                    "desc"   => "Malformed email with double @ should be INVALID.",
                    "marks"  => 2.5
                ],
                // 3: phone extraction and normalization
                [
                    "input"  => "phone\nCall me at +1-202-555-0173 or (202) 555-0125. Office: 202.555.0186\n",
                    "output" => "+12025550173\n2025550125\n2025550186\n",
                    "desc"   => "Extract three phone numbers and normalize (preserve leading +).",
                    "marks"  => 2.5
                ],
                // 4: password strength (weak case)
                [
                    "input"  => "password\nabc123\n",
                    "output" => "WEAK\n",
                    "desc"   => "Password too short and missing required character classes -> WEAK.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (strpos($desc, "email") !== false) {
                if (strpos($answer, '@') !== false && (strpos($answer, 'valid') !== false || strpos($answer, 'regex') !== false || strpos($answer, 're') !== false)) $passed = true;
                if (strpos($answer, 'invalid') !== false && (strpos($answer, '@@') !== false || strpos($answer, 'invalid') !== false)) $passed = true;
            } elseif (strpos($desc, "phone") !== false) {
                if (strpos($answer, 'phone') !== false || strpos($answer, '+') !== false || strpos($answer, 'extract') !== false) $passed = true;
            } elseif (strpos($desc, "password") !== false) {
                if (strpos($answer, 'strong') !== false || strpos($answer, 'weak') !== false || strpos($answer, 'regex') !== false) $passed = true;
            } else {
