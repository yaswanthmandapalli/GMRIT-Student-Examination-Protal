...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Array Manipulations : Matrix Operations ---
        $question = [
            "topic" => "Array Manipulations: Matrix Operations (Transpose, Symmetry, Multiplication)",
            "question" => "Input format varies by operation. The first token (single word) indicates the operation and will be one of:\n  - transpose\n  - symmetric\n  - multiply\n\nFor 'transpose' and 'symmetric':\n  - Next line contains two integers r c (rows, columns).\n  - Next r lines each contain c integers (space-separated) representing the matrix.\n\nFor 'multiply':\n  - Next line: r1 c1\n  - Next r1 lines: each c1 integers (matrix A)\n  - Next line: r2 c2\n  - Next r2 lines: each c2 integers (matrix B)\n\nRequired behavior:\n- If operation is 'transpose': print the transpose of the given r x c matrix. Each row on its own line, numbers separated by single spaces, then a final newline.\n- If operation is 'symmetric': if the matrix is square (r == c) and equals its transpose print 'YES' followed by a newline; otherwise print 'NO' followed by a newline.\n- If operation is 'multiply': if c1 != r2 print exactly 'INVALID' followed by a newline. Otherwise print the resulting product matrix (r1 x c2), one row per line, numbers separated by single spaces, and a final newline.\n\nOutput must not have trailing spaces on any line. Use integer arithmetic throughout.",
            "explanation" => "Tests array manipulation: computing matrix transpose, checking symmetry for square matrices, and performing matrix multiplication with dimension checks.",
            "test_cases" => [
                // Test 1: transpose (2x3 -> 3x2)
                [
                    "input"  => "transpose\n2 3\n1 2 3\n4 5 6\n",
                    "output" => "1 4\n2 5\n3 6\n",
                    "desc"   => "Transpose of a 2x3 matrix -> 3x2 result.",
                    "marks"  => 2.5
                ],
                // Test 2: symmetric (3x3 symmetric matrix)
                [
                    "input"  => "symmetric\n3 3\n1 2 3\n2 5 6\n3 6 9\n",
                    "output" => "YES\n",
                    "desc"   => "Square symmetric 3x3 matrix -> YES.",
                    "marks"  => 2.5
                ],
                // Test 3: multiply (2x3 * 3x2)
                [
                    "input"  => "multiply\n2 3\n1 2 3\n4 5 6\n3 2\n7 8\n9 10\n11 12\n",
                    "output" => "58 64\n139 154\n",
                    "desc"   => "Multiply compatible matrices (2x3 * 3x2) -> 2x2 product.",
                    "marks"  => 2.5
                ],
                // Test 4: multiply with incompatible dimensions -> INVALID
                [
                    "input"  => "multiply\n2 2\n1 2\n3 4\n3 2\n1 2\n3 4\n5 6\n",
                    "output" => "INVALID\n",
                    "desc"   => "Multiply with c1 != r2 should print INVALID.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (strpos($desc, "transpose") !== false) {
                if (strpos($answer, 'transpose') !== false || strpos($answer, 'zip') !== false || strpos($answer, 'matrix') !== false) $passed = true;
            } elseif (strpos($desc, "symmetric") !== false) {
                if (strpos($answer, 'symmetric') !== false || strpos($answer, 'transpose') !== false) $passed = true;
            } elseif (strpos($desc, "multiply") !== false) {
                if (strpos($answer, 'multiply') !== false || strpos($answer, 'dot') !== false) $passed = true;
            } else {
