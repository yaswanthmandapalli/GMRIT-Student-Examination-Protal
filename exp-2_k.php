...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Control Statements: Implementing Fibonacci Series and Advanced List Operations ---
        $question = [
            "topic" => "Control Statements: Fibonacci Series & Advanced List Operations",
            "question" => "Read input from standard input as follows:\n- First line: a single integer n (number of Fibonacci terms to produce).\n- Second line: zero or more integers separated by spaces (a list on which to perform advanced list operations).\n\nTask:\n1) If n < 1, print exactly: INVALID (followed by a newline) and exit.\n2) Otherwise, print the first n Fibonacci numbers (starting with 0, 1, ...) separated by a single space on the first line, followed by a newline.\n3) On the second line, print the given list after removing duplicates and sorting the remaining values in ascending order, separated by a single space. If the input list is empty, print an empty line (just a newline).\n\nNotes:\n- Fibonacci sequence starts: 0 1 1 2 3 5 8 ...\n- Input may contain negative integers in the list. They should be treated normally.\n- Output must match spacing exactly (single space between numbers, no trailing spaces).",
            "explanation" => "This tasks control flow (conditional, loops), correct Fibonacci generation, and list operations: deduplication and sorting.",
            "test_cases" => [
                // 1: typical case
                [
                    "input"  => "5\n3 1 4 1 5\n",
                    "output" => "0 1 1 2 3\n1 3 4 5\n",
                    "desc"   => "n=5 -> first 5 Fibonacci numbers; list dedup+sort from '3 1 4 1 5'.",
                    "marks"  => 2.5
                ],
                // 2: single term, all same in list
                [
                    "input"  => "1\n7 7 7\n",
                    "output" => "0\n7\n",
                    "desc"   => "n=1 -> single Fibonacci term (0); list becomes single unique value 7.",
                    "marks"  => 2.5
                ],
                // 3: invalid n
                [
                    "input"  => "0\n1 2 3\n",
                    "output" => "INVALID\n",
                    "desc"   => "n=0 invalid -> print INVALID.",
                    "marks"  => 2.5
                ],
                // 4: larger n and negative numbers in list
                [
                    "input"  => "7\n-1 -2 -1 0\n",
                    "output" => "0 1 1 2 3 5 8\n-2 -1 0\n",
                    "desc"   => "n=7 -> first 7 Fibonacci numbers; list dedup+sort with negatives.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (stripos($desc, "hello") !== false) {
                ...
                // check for 'print' or 'fib' strings as loose heuristic
                if (strpos($answer, 'print') !== false || strpos($answer, 'fib') !== false || strpos($answer, 'fibonacci') !== false) $passed = true;
            }
            if ($passed) {
