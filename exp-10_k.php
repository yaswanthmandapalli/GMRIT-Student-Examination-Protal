...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Functional Programming — Map, Filter, Reduce ---
        $question = [
            "topic" => "Functional Programming: Map, Filter, Reduce Operations",
            "question" => "First token (single word) selects the operation: one of 'map', 'filter', 'reduce'.\n\nInput / behavior details:\n\n1) 'map'\n   - Next line: zero or more space-separated integers (the list). If the line is empty, treat it as an empty list.\n   - Requirement: apply a mapping function that squares each element (x -> x*x). Print the transformed list elements in the same order separated by single spaces followed by a newline. If the input list is empty print an empty line.\n\n2) 'filter'\n   - Next line: zero or more space-separated integers (the list).\n   - Requirement: filter the list to keep only even numbers (value % 2 == 0). Print the remaining numbers in their original order separated by single spaces followed by a newline. If none match, print an empty line.\n\n3) 'reduce'\n   - Next line: a reduction operation token and the list separated by a newline. Specifically:\n       * First (after 'reduce') line: operation token — one of 'sum' or 'product'.\n       * Next line: zero or more space-separated integers (the list).\n   - Requirement: if operation is 'sum', compute the sum of the list and print the integer result followed by a newline. If operation is 'product', compute the product of the list and print the integer result followed by a newline. For an empty list: define sum -> 0, product -> 1.\n\nBe exact with spacing (single spaces between numbers) and newlines. Use integer arithmetic.",
            "explanation" => "Tests functional-style list transformations: mapping (squares), filtering (evens), and reducing (sum/product) with correct empty-list identities.",
            "test_cases" => [
                // 1: map squares typical
                [
                    "input"  => "map\n1 2 3 4\n",
                    "output" => "1 4 9 16\n",
                    "desc"   => "Map: square each number in [1,2,3,4].",
                    "marks"  => 2.5
                ],
                // 2: filter evens typical
                [
                    "input"  => "filter\n1 2 3 4 5 6\n",
                    "output" => "2 4 6\n",
                    "desc"   => "Filter: keep even numbers from the list.",
                    "marks"  => 2.5
                ],
                // 3: reduce sum typical
                [
                    "input"  => "reduce\nsum\n5 1 4 2 3\n",
                    "output" => "15\n",
                    "desc"   => "Reduce sum over [5,1,4,2,3] -> 15.",
                    "marks"  => 2.5
                ],
                // 4: reduce product on empty list -> identity 1
                [
                    "input"  => "reduce\nproduct\n\n",
                    "output" => "1\n",
                    "desc"   => "Reduce product on empty list should return identity 1.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (strpos($desc, "map") !== false) {
                if (strpos($answer, 'map') !== false || strpos($answer, 'lambda') !== false || strpos($answer, 'square') !== false) $passed = true;
            } elseif (strpos($desc, "filter") !== false) {
                if (strpos($answer, 'filter') !== false || strpos($answer, '% 2') !== false || strpos($answer, 'even') !== false) $passed = true;
            } elseif (strpos($desc, "reduce") !== false) {
                if (strpos($answer, 'reduce') !== false || strpos($answer, 'sum') !== false || strpos($answer, 'product') !== false) $passed = true;
            } else {
