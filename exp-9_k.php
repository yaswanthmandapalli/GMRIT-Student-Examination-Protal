...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Exploring Default & Keyword Arguments, Variable Scopes, Lambda Functions ---
        $question = [
            "topic" => "Default & Keyword Arguments, Variable Scopes, Lambda Functions",
            "question" => "First token (single word) selects the operation: one of 'defaults', 'scope', 'lambda'.\n\nOperations and input format:\n\n1) 'defaults'\n   - Next line: one or two integers separated by space: a [b]\n   - Requirement: treat b as optional with default value 10. Compute a + b and print the integer result followed by a newline.\n   - Example: input 'defaults\\n5 7' -> output '12'. Input 'defaults\\n5' -> output '15' (since b defaults to 10).\n\n2) 'scope'\n   - Next line: single integer x.\n   - Requirement: demonstrate variable scope semantics. Your program should:\n       * set a global variable g = x\n       * define a function add_local(val) that does val += 1 and returns val (does not modify g)\n       * define a function add_global() that declares 'global g' and increments g by 1\n       * call add_local(g) but do NOT use its return to set g, then print the current global g on the first line\n       * then call add_global() and print the current global g on the second line\n     So for input x the output lines should be:\n       <g before global modification>\\n\n       <g after add_global()>\\n\n     Example: input 'scope\\n5' -> output '5\\n6\\n'\n\n3) 'lambda'\n   - Next line: zero or more space-separated integers (a list). If empty line, treat it as empty list.\n   - Requirement: use a lambda (or equivalent) to transform each element to its square, and print the squared numbers in the same order separated by single spaces followed by a newline. If the list is empty print an empty line.\n   - Example: input 'lambda\\n1 2 3' -> output '1 4 9\\n'.\n\nBe exact with spacing and newlines. Use integer arithmetic where applicable.",
            "explanation" => "Tests understanding of default and optional parameters, the difference between local and global variable scopes, and applying lambda/map-style transformations to lists.",
            "test_cases" => [
                // Test 1: defaults with explicit b
                [
                    "input"  => "defaults\n5 7\n",
                    "output" => "12\n",
                    "desc"   => "defaults operation with explicit b -> 5 + 7 = 12",
                    "marks"  => 2.5
                ],
                // Test 2: defaults with missing b (use default 10)
                [
                    "input"  => "defaults\n5\n",
                    "output" => "15\n",
                    "desc"   => "defaults operation with missing b -> 5 + 10(default) = 15",
                    "marks"  => 2.5
                ],
                // Test 3: scope demonstration
                [
                    "input"  => "scope\n5\n",
                    "output" => "5\n6\n",
                    "desc"   => "scope: print global before and after increment via add_global()",
                    "marks"  => 2.5
                ],
                // Test 4: lambda squares
                [
                    "input"  => "lambda\n1 2 3\n",
                    "output" => "1 4 9\n",
                    "desc"   => "lambda: square each element and print in order",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (strpos($desc, "defaults") !== false) {
                if (strpos($answer, 'default') !== false || strpos($answer, 'def') !== false || strpos($answer, '+') !== false) $passed = true;
            } elseif (strpos($desc, "scope") !== false) {
                if (strpos($answer, 'global') !== false || strpos($answer, 'local') !== false || strpos($answer, 'global g') !== false) $passed = true;
            } elseif (strpos($desc, "lambda") !== false) {
                if (strpos($answer, 'lambda') !== false || strpos($answer, 'map') !== false || strpos($answer, 'square') !== false) $passed = true;
            } else {
