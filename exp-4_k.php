...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Array Manipulations: Intersection, Kth Largest, Maximum Subarray ---
        $question = [
            "topic" => "Array Manipulations: Intersection, Kth Largest, Maximum Subarray",
            "question" => "First line: an operation token (one of: 'intersection', 'kth', 'maxsubarray').\n\nIf operation == 'intersection':\n  - Next line: two integers m n (sizes of two lists).\n  - Next line: m integers (first list).\n  - Next line: n integers (second list).\n  - Output: the intersection of both lists (unique values only), printed in ascending order separated by single spaces followed by a newline. If intersection is empty, print an empty line (just a newline).\n\nIf operation == 'kth':\n  - Next line: single integer k (1-based).\n  - Next line: space-separated integers (the list).\n  - Output: the k-th largest element (1 => largest). If k is out of range (k < 1 or k > length), print 'INVALID' followed by a newline.\n\nIf operation == 'maxsubarray':\n  - Next line: space-separated integers (the list; may contain negatives).\n  - Output: a single integer — the maximum subarray sum (Kadane's result) followed by a newline.\n\nAll outputs must match spacing exactly (single spaces, no trailing spaces). Use integer arithmetic.",
            "explanation" => "Tests list operations: computing intersection (unique+sorted), selecting k-th largest element with bounds checking, and computing maximum contiguous subarray sum (Kadane).",
            "test_cases" => [
                // 1: intersection typical
                [
                    "input"  => "intersection\n5 6\n1 2 3 4 5\n3 5 7 3 2 1\n",
                    "output" => "1 2 3 5\n",
                    "desc"   => "Intersection of two lists -> unique sorted values 1 2 3 5.",
                    "marks"  => 2.5
                ],
                // 2: kth largest typical
                [
                    "input"  => "kth\n3\n5 1 4 2 3\n",
                    "output" => "3\n",
                    "desc"   => "3rd largest in [5,1,4,2,3] is 3.",
                    "marks"  => 2.5
                ],
                // 3: kth invalid (out of range)
                [
                    "input"  => "kth\n6\n2 2 3\n",
                    "output" => "INVALID\n",
                    "desc"   => "k out of range -> INVALID.",
                    "marks"  => 2.5
                ],
                // 4: maxsubarray with negatives
                [
                    "input"  => "maxsubarray\n-2 -3 4 -1 -2 1 5 -3\n",
                    "output" => "7\n",
                    "desc"   => "Maximum subarray sum is 7 for subarray [4, -1, -2, 1, 5].",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
    ...
    // original fallback replaced by updated question for this assignment
    $question = [
        "topic" => "Array Manipulations: Intersection, Kth Largest, Maximum Subarray",
        "question" => "First line: an operation token (one of: 'intersection', 'kth', 'maxsubarray').\n\nIf operation == 'intersection':\n  - Next line: two integers m n (sizes of two lists).\n  - Next line: m integers (first list).\n  - Next line: n integers (second list).\n  - Output: the intersection of both lists (unique values only), printed in ascending order separated by single spaces followed by a newline. If intersection is empty, print an empty line (just a newline).\n\nIf operation == 'kth':\n  - Next line: single integer k (1-based).\n  - Next line: space-separated integers (the list).\n  - Output: the k-th largest element (1 => largest). If k is out of range (k < 1 or k > length), print 'INVALID' followed by a newline.\n\nIf operation == 'maxsubarray':\n  - Next line: space-separated integers (the list; may contain negatives).\n  - Output: a single integer — the maximum subarray sum (Kadane's result) followed by a newline.\n\nAll outputs must match spacing exactly (single spaces, no trailing spaces). Use integer arithmetic.",
        "explanation" => "Tests list operations: computing intersection (unique+sorted), selecting k-th largest element with bounds checking, and computing maximum contiguous subarray sum (Kadane).",
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            if (strpos($desc, "intersection") !== false) {
                if (strpos($answer, 'set') !== false || strpos($answer, 'intersection') !== false || strpos($answer, ' & ') !== false) $passed = true;
            } elseif (strpos($desc, "kth") !== false) {
                if (strpos($answer, 'kth') !== false || strpos($answer, 'largest') !== false || strpos($answer, 'sort') !== false) $passed = true;
            } elseif (strpos($desc, "maxsubarray") !== false) {
                if (strpos($answer, 'kadane') !== false || strpos($answer, 'max sub') !== false || strpos($answer, 'maximum') !== false) $passed = true;
            } else {
