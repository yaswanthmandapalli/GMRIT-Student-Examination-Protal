...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: String Manipulation: Acronyms, Valid Parentheses, Anagram Check, Common Prefix ---
        $question = [
            "topic" => "String Manipulation: Acronyms, Parentheses, Anagram, Common Prefix",
            "question" => "First line: an operation token — one of: 'acronym', 'parentheses', 'anagram', 'prefix'.\n\nOperation formats and required outputs:\n\n1) 'acronym'\n   - Next line: a phrase (one or more words, separated by whitespace).\n   - Requirement: create the acronym by taking the first letter of each word (in order), uppercase them, and print as a single token followed by a newline. Example: 'Portable Network Graphics' -> 'PNG'.\n\n2) 'parentheses'\n   - Next line: a string that may contain parentheses characters '()','[]','{}'.\n   - Requirement: print 'YES' followed by newline if the parentheses in the string are balanced and properly nested (consider only () [] {} pairs); otherwise print 'NO' followed by newline.\n\n3) 'anagram'\n   - Next line: two tokens separated by whitespace (two words).\n   - Requirement: if the two tokens are anagrams of each other (case-insensitive, considering only letters and digits), print 'YES' followed by newline; else print 'NO' followed by newline.\n\n4) 'prefix'\n   - Next line: one or more words separated by spaces.\n   - Requirement: print the longest common prefix string shared by all words (exact characters, case-sensitive), followed by a newline. If there is no common prefix, print an empty line (just a newline).\n\nBe exact with spacing (single spaces where required) and newlines.",
            "explanation" => "Tests basic string processing: building acronyms, checking balanced parentheses, determining anagrams, and computing longest common prefix.",
            "test_cases" => [
                // 1: acronym typical
                [
                    "input"  => "acronym\nPortable Network Graphics\n",
                    "output" => "PNG\n",
                    "desc"   => "Create acronym from multi-word phrase.",
                    "marks"  => 2.5
                ],
                // 2: parentheses balanced
                [
                    "input"  => "parentheses\n[{()}]\n",
                    "output" => "YES\n",
                    "desc"   => "Balanced nested brackets should return YES.",
                    "marks"  => 2.5
                ],
                // 3: anagram check (case-insensitive)
                [
                    "input"  => "anagram\nlisten silent\n",
                    "output" => "YES\n",
                    "desc"   => "listen and silent are anagrams (case-insensitive, letters only).",
                    "marks"  => 2.5
                ],
                // 4: common prefix
                [
                    "input"  => "prefix\nflower flow flight\n",
                    "output" => "fl\n",
                    "desc"   => "Longest common prefix among words is 'fl'.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
    ...
    // fallback question (same as above)
    $question = [
        "topic" => "String Manipulation: Acronyms, Parentheses, Anagram, Common Prefix",
        "question" => "First line: an operation token — one of: 'acronym', 'parentheses', 'anagram', 'prefix'.\n\nOperation formats and required outputs:\n\n1) 'acronym'\n   - Next line: a phrase (one or more words, separated by whitespace).\n   - Requirement: create the acronym by taking the first letter of each word (in order), uppercase them, and print as a single token followed by a newline. Example: 'Portable Network Graphics' -> 'PNG'.\n\n2) 'parentheses'\n   - Next line: a string that may contain parentheses characters '()','[]','{}'.\n   - Requirement: print 'YES' followed by newline if the parentheses in the string are balanced and properly nested (consider only () [] {} pairs); otherwise print 'NO' followed by newline.\n\n3) 'anagram'\n   - Next line: two tokens separated by whitespace (two words).\n   - Requirement: if the two tokens are anagrams of each other (case-insensitive, considering only letters and digits), print 'YES' followed by newline; else print 'NO' followed by newline.\n\n4) 'prefix'\n   - Next line: one or more words separated by spaces.\n   - Requirement: print the longest common prefix string shared by all words (exact characters, case-sensitive), followed by a newline. If there is no common prefix, print an empty line (just a newline).\n\nBe exact with spacing (single spaces where required) and newlines.",
        "explanation" => "Tests basic string processing: building acronyms, checking balanced parentheses, determining anagrams, and computing longest common prefix.",
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            ...
            $passed = false;
            // loose heuristics
            if (strpos($desc, "acronym") !== false) {
                if (strpos($answer, 'upper') !== false || strpos($answer, 'acron') !== false || strpos($answer, 'split') !== false) $passed = true;
            } elseif (strpos($desc, "parentheses") !== false || strpos($desc, "bracket") !== false) {
                if (strpos($answer, 'stack') !== false || strpos($answer, 'balanced') !== false || strpos($answer, 'pop') !== false) $passed = true;
            } elseif (strpos($desc, "anagram") !== false) {
                if (strpos($answer, 'sorted') !== false || strpos($answer, 'anagram') !== false) $passed = true;
            } elseif (strpos($desc, "prefix") !== false) {
                if (strpos($answer, 'prefix') !== false || strpos($answer, 'common') !== false) $passed = true;
            } else {
