...
if (file_exists($problem_file)) {
    ...
    if (is_array($decoded) && isset($decoded['question'])) {
        ...
        // --- Updated question for: Exception Handling: Custom Exceptions ---
        $question = [
            "topic" => "Exception Handling: Custom Exceptions",
            "question" => "Write a single Python program that reads from standard input. The first line is an operation token (single word) selecting one of these behaviors:\n\n1) 'raise_and_catch'\n   - No further input.\n   - Requirement: Define a custom exception class MyError(Exception). In code, raise MyError inside a try block and catch it; when caught print exactly: Handled MyError\\n\n\n2) 'custom_message'\n   - Next line: a message string (may contain spaces).\n   - Requirement: Define MyError that accepts a message. Raise MyError(message) and catch it; when caught print exactly: Error: <message>\\n\n\n3) 'chain_exception'\n   - No further input.\n   - Requirement: Cause a built-in exception (e.g. ZeroDivisionError) inside a try block, catch it and raise MyError from the original exception (using \"raise MyError() from e\"). Then catch MyError and print exactly: Chained: <OrigExceptionName>\\n where <OrigExceptionName> is the class name of the original exception (for example ZeroDivisionError).\n\n4) 'validate'\n   - Next line: single integer (n)\n   - Requirement: Define InvalidValue(MyError). If n is negative, raise InvalidValue; catch InvalidValue and print exactly: Invalid\\n. If n is >= 0, print exactly: Valid\\n.\n\nBe exact with capitalization, single spaces and trailing newlines. Implementations must use custom exception classes and proper try/except (and exception chaining for 'chain_exception').",
            "explanation" => "Students must demonstrate creating custom exceptions, raising and catching them, passing messages through exceptions, using exception chaining (raise ... from ...), and using custom exceptions for validation.",
            "test_cases" => [
                [
                    "input"  => "raise_and_catch\n",
                    "output" => "Handled MyError\n",
                    "desc"   => "Raise MyError and catch it, printing 'Handled MyError'.",
                    "marks"  => 2.5
                ],
                [
                    "input"  => "custom_message\nFile not found\n",
                    "output" => "Error: File not found\n",
                    "desc"   => "Raise MyError with message 'File not found' and print 'Error: File not found'.",
                    "marks"  => 2.5
                ],
                [
                    "input"  => "chain_exception\n",
                    "output" => "Chained: ZeroDivisionError\n",
                    "desc"   => "Raise a ZeroDivisionError then raise MyError from it; on catching MyError print original exception class name.",
                    "marks"  => 2.5
                ],
                [
                    "input"  => "validate\n-5\n",
                    "output" => "Invalid\n",
                    "desc"   => "Validate negative number -> raise InvalidValue and print 'Invalid'.",
                    "marks"  => 2.5
                ]
            ],
            "marks" => 10
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['earned'])) {
        ...
        foreach ($question['test_cases'] as $case) {
            $desc = $case['desc'] ?? '';
            $passed = false;
            // very loose heuristics to avoid zero-credit if client grading unavailable
            if (strpos($desc, "Handled MyError") !== false || strpos($desc, "raise myerror") !== false) {
                if (strpos($answer, 'myerror') !== false && strpos($answer, 'try') !== false) $passed = true;
            } elseif (strpos($desc, "Error: File not found") !== false || strpos($desc, "custom_message") !== false) {
                if (strpos($answer, 'myerror') !== false && strpos($answer, 'message') !== false) $passed = true;
            } elseif (strpos($desc, "Chained") !== false) {
                if (strpos($answer, 'from') !== false || strpos($answer, 'zero') !== false) $passed = true;
            } elseif (strpos($desc, "Invalid") !== false) {
                if (strpos($answer, 'invalid') !== false || strpos($answer, 'raise') !== false) $passed = true;
            } else {
